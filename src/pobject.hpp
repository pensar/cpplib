...
namespace pensar_digital
{
    namespace cpplib
    {
        ...
        {
        public:
            typedef pd::Factory<PObject, Id> Factory;

        private:
            inline static Factory mfactory = { 3, 10, NULL_ID }; //!< Member variable "factory"
            /// \brief Class PObject::Data is compliant with the StdLayoutTriviallyCopyable concept. 
        ...
        protected:
            ...
            S ObjXMLPrefix() const noexcept { return "<object class_name = \"" + class_name() + "\" id = \"" + PObject::to_string() + "\""; }
            ...
            /// \see equals
            ///
            virtual bool _equals(const PObject& o) const { return (mdata.mid == o.mdata.mid); }
        ...
        public:
            ...
            /// Default constructor.
            PObject(const Id& id = NULL_ID, const IO_Mode mode = BINARY) noexcept : mdata(id)
            {
            };

            /// Copy constructor
            /// \param other PObject to copy from
            PObject(const PObject& o) { assign(o); }

            /// Move constructor
            PObject(PObject&& o) noexcept { assign(o); }
            ...
            PObject& assign(const PObject& o) noexcept { mdata.mid = o.mdata.mid; return *this; }
            ...
            // Clone method. 
            PObjectPtr clone() const noexcept { return pd::clone<PObject>(*this, mdata.mid); }
            ...
            bool equals(const PObject& o) const noexcept
            {
                if (get_hash() !=
                    o.get_hash())
                    return false;
            ...
            bool operator == (const PObject& o) const { return   equals(o); }
            bool operator != (const PObject& o) const { return !equals(o); }

            PObject& from_json(const S& sjson);
            ...
            /// Assignment operator
            /// \param o PObject to assign from
            /// \return A reference to this
            PObject& operator=(const PObject& o) noexcept { return assign(o); }

            /// Move assignment operator
            PObject& operator=(PObject&& o) noexcept { return assign(o); }

            friend void from_json(const Json& j, PObject& o);
            ...
            inline static Factory::P get(const Json& j)
            {
                ...
                if (json_class != pd::class_name<PObject, char>())
                    throw std::runtime_error("Invalid class name: " + pd::class_name<PObject, char>());
            ...
            {
                Json j;
                Factory::P ptr = get(pd::id<PObject>(sjson, &j));
        ...
        extern void to_json(Json& j, const PObject& o);
        extern void from_json(const Json& j, PObject& o);
        //extern PObject nlohmann::from_json (const Json& j);

        inline std::istream& operator >> (std::istream& is, PObject& o)
        {
            return o.read(is, TEXT);
        }

        inline std::ostream& operator << (std::ostream& os, const PObject& o)
        {
            return o.write(os, TEXT);
        }

        inline PObject& operator >> (const S& sjson, PObject& o) { return o.from_json(sjson); }

        inline std::istream& operator >> (std::istream& is, PObjectPtr o) { return is >> *o; }
        inline std::ostream& operator << (std::ostream& os, const PObjectPtr o) { return os << *o; }

        static_assert (std::is_move_constructible_v <PObject>);
        static_assert (std::is_move_assignable_v    <PObject>);
